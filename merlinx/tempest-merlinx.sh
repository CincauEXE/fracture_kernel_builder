...
# Kernel Sources
git clone --depth=1 https://github.com/CincauEXE/kernel_redmi_mt6768 -b base-r-oss-tmpst tmpst
...
IMAGE=$(pwd)/tmpst/out/arch/arm64/boot/Image.gz-dtb
HEADCOMMITID="$(git log --pretty=format:'%h' -n1)"
DATE=$(date +"%F-%S")
DATE2=$(date +"%m%d")
START=$(date +"%s")
PATH="${PATH}:${CLANG_ROOTDIR}/bin:$(pwd)/gcc/bin:$(pwd)/gcc32/bin:${PATH}"
DTB=$(pwd)/tmpst/out/arch/arm64/boot/dts/mediatek/mt6768.dtb
DTBO=$(pwd)/tmpst/out/arch/arm64/boot/dtbo.img
